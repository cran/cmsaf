...
- the toolbox is now one shiny app independent of the R console
- only the data of the currently visible timestep is loaded during the 'visualize' step
- add region plots (plotting the data of only one country)
- improve overall usability (scrolling, overflow)
- add a 'user directory' which is basically a separate working directory for only the toolbox. Its path is stored in a config file in the user's home directory
- let users choose the 'user directory' and a grid for remapping when the toolbox is first started
- add possibility to download an auxiliary file from inside the toolbox
- generated images get a data source label
- color bar schemes can be selected using png previews

# 1.9.5

timsum: bug fix - zero instead of NA
        new na.rm option of treatment of NA values
box_mergetime, cmsaf.cat, levbox_mergetime: bug fix - now merging of files with more than 
                                            one time step is allowed
...
all operators with var input: no case sensitivity for var and if var not found default 
                              var is used; operator name is included in output NetCDF
...
trend: bug fix in trend calcualtion, which could lead to wrong trend values
...
Prep.Data.R: remove functionality to avoid overwriting; now capabale to handle daily and hourly data;

...
########## start value is NA ##########
#TODO this should remove the period including the end date, but doesn't include the end date
file_out <- tempfile_nc()
extract.period("SIS",
               NA,
               "2000-03-01",
...
test_that("data is correct", {
  ...
  expected_data <- c(254:272, 250:272)
  expected <- aperm(array(expected_data, dim = c(3, 3, 2)), c(2, 1, 3))
...
test_that("coordinates are correct", {
  ...
  actual <- ncvar_get(file, "time")
  expect_equal(actual, array(c(151200, 151920)))
})

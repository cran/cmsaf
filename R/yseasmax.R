...
# create netcdf
  ...
  if (nc34==4){
    ...
    cmsaf_info <- (paste("cmsaf::yseasmax for variable ",var,sep=""))
  ...
  # get data and calculate multi-year seasonal maxima
    ...
    for (j in 1:4){
      ...
      if (j==4)(mon_dummy <- dummy_aut)
	    if (sum(is.finite(mon_dummy))>=3){
	      ...
	      cat("\r","apply multi-year seasonal maximum ",count," of 4",sep="")
	      max_data <- do.call(pmax, c(na.rm=T,lapply(seq(1:dim(dum_dat)[3]), function(i) dum_dat[,,i])))
	      max_data[is.na(max_data)] <- v_missing_value
	      tdum <- min(time1[mon_dummy],na.rm=T)
	      tbnds[1,1] <- min(time1[mon_dummy],na.rm=T)
	      tbnds[2,1] <- max(time1[mon_dummy],na.rm=T)
	      ncvar_put(ncnew,var1,max_data,start=c(1,1,count),count=c(-1,-1,1))
	      ncvar_put(ncnew,t,tdum,start=count,count=1)
	      ncvar_put(ncnew,var2,tbnds,start=c(1,count),count=c(-1,1))
	      count <- count+1
	    } else {cat("Not enough data to calculate a seasonal maximum!","\n")}
    }

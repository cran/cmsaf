...
# create netcdf
  ...
  if (nc34==4){
    ...
    cmsaf_info <- (paste("cmsaf::ymonsd for variable ",var,sep=""))
    ...
    # get data and calculate multi-year standard deviation
    ...
    for (j in 1:length(ml)){
      ...
      cat("\r","multi-year monthly standard deviation ",j,sep="")
	    sd_data <- apply(dum_dat,c(1,2),sd,na.rm=T)
	    sd_data[is.na(sd_data)] <- v_missing_value
      ncvar_put(ncnew,var1,sd_data,start=c(1,1,j),count=c(-1,-1,1))
    }

...
# create netcdf
  ...
  if (nc34==4){
    ...
    cmsaf_info <- (paste("cmsaf::multimonmean of month(s): ",dummy_month," for variable ",var,sep=""))
  ...
  for (i in 1:length(yl)){
    ...
    for (j in 1:length(month)){
      ...
      if(length(dum)>0){
        ...
        dumdum_dat <- rowMeans(dumdum_dat,dims=2,na.rm=T)
        dum_dat[,,j] <- dumdum_dat
      }
    }
    if(sum(!is.na(dum_dat))!=0){
      cat("\r","apply multi monthly mean ",count," of ",(length(yl)),sep="")

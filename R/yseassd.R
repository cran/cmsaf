...
# create netcdf
  ...
  if (nc34==4){
    ...
    cmsaf_info <- (paste("cmsaf::yseassd for variable ",var,sep=""))
  ...
  # get data and calculate multi-year seasonal standard deviation
    ...
    for (j in 1:4){
      ...
      if (j==4)(mon_dummy <- dummy_aut)
	    if (sum(is.finite(mon_dummy))>=3){
	      ...
	      cat("\r","apply multi-year seasonal standard deviation ",count," of 4",sep="")
	      sd_data <- apply(dum_dat,c(1,2),sd,na.rm=T)
	      sd_data[is.na(sd_data)] <- v_missing_value
	      tdum <- min(time1[mon_dummy],na.rm=T)
	      tbnds[1,1] <- min(time1[mon_dummy],na.rm=T)
	      tbnds[2,1] <- max(time1[mon_dummy],na.rm=T)
	      ncvar_put(ncnew,var1,sd_data,start=c(1,1,count),count=c(-1,-1,1))
	      ncvar_put(ncnew,t,tdum,start=count,count=1)
	      ncvar_put(ncnew,var2,tbnds,start=c(1,count),count=c(-1,1))
	      count <- count+1
	    } else {cat("Not enough data to calculate a seasonal standard deviation!","\n")}
    }

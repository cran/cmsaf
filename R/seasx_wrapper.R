seasx_wrapper <- function(op, var, infile, outfile, nc34, overwrite, verbose) {
  ...
  switch(op,
         cmsaf_info <- paste0("cmsaf::seasmean for variable ", file_data$variable$name),
         cmsaf_info <- paste0("cmsaf::seassum for variable ", file_data$variable$name)
  )

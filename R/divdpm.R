...
# get file information
  ...
  cat("get file information of infile", "\n")
...
# extract time information
 ...
 # create netcdf
  ...
  if (nc34==4){
    ...
    cmsaf_info <- (paste("cmsaf::divdpm for variable ",var,sep=""))
    ...
    # multiply each timestep with number of days per month
	...
	# get days per month
	leap = 0
	if (years[i] %% 4 == 0 & (years[i] %% 100 != 0 | years[i] %% 400 == 0)){leap = 1}
	dpm <- (switch(mon[i],
         '01' = 31,
         '02' = 28 + leap,
         '03' = 31,
	  ...
	  cat("\r","divide timestep ",i," of ",length(time1),sep="")
	  if (dpm!=0){dum_dat <- dum_dat/dpm}

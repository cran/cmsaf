...
#'extract.period("SIS", "2001-01-01", "2002-01-01", "CMSAF_example_file.nc",
#'  "CMSAF_example_file_extract.period.nc")
#'
#'unlink(c("CMSAF_example_file.nc", "CMSAF_example_file_extract.period.nc"))
extract.period <- function(var, start, end, infile, outfile, nc34 = 4,
  ...
  if (!length(period) || length(period) == length(date.time)){
  ...
  cmsaf_info <- (paste0("cmsaf::extract.period for variable ", file_data$variable$name))

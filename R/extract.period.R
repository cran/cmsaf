extract.period <-
function(var,start,end,infile,outfile,nc34=3){
...
# extract time information
 ...
 if (length(period)>=1&length(period)!=length(date.time)){
 ...
 # create netcdf
  ...
  if (nc34==4){
    ...
    cmsaf_info <- (paste("cmsaf::extract.period for variable ",var,sep=""))
    ...
    # remove desired period from infile

      id <- nc_open(infile)
      count <- 1
  
      for (i in 1:length(period)){
  	    dum_dat <- ncvar_get(id,var,start=c(1,1,period[i]),count=c(-1,-1,1))
	      dum_dat[is.na(dum_dat)] <- v_missing_value
	      ncvar_put(ncnew,var1,dum_dat,start=c(1,1,count),count=c(-1,-1,1))
	      ncvar_put(ncnew,t,time1[period[i]], start=count, count=1)
	      if ("time_bnds" %in% varnames){
	        ncvar_put(ncnew,var2,tbnds1[,period[i]],start=c(1,count),count=c(-1,1))
	      }
	      count <- count+1
      }
      nc_close(id)
 ...
 } else { cat("No match. Dates are:",as.character(date.time), "\n")}
